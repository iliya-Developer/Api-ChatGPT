...
tor_lock = Lock()
last_renew_time = 0
MIN_RENEW_INTERVAL = 10

def renew_tor_ip():
    global last_renew_time
    
    current_time = time.time()
    if current_time - last_renew_time < MIN_RENEW_INTERVAL:
        return
    ...
    with tor_lock:
        ...
        try:
            with Controller.from_port(port=CONTROL_PORT) as controller:
                ...
                print("IP تور با موفقیت تغییر کرد")
                time.sleep(1)
        except Exception as e:
            print(f"خطا در تغییر IP تور: {str(e)}")
...
def ask_gpt(message, history):
    try:
        ...
        try:
            response = req.post(api, headers=headers, json=payload, stream=True, timeout=60)
            answer = ""
